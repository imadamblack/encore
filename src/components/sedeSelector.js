...
export default function ModalSedeSelector({onClose, onSelect}) {
  const sedes = [
    ...
    {value: 'gdl-a', name: 'Guadalajara Aeropuerto'},
    {value: 'mty', name: 'Monterrey'},
        ...
        <div className="relative bg-gray-100 rounded-2xl p-8 lg:w-1/2 max-w-[40rem] mx-auto shadow-xl">
          ...
          <div className="grid grid-cols-1 gap-4">
            {sedes.map((sede) => (
              <button
                key={sede.value}
                className="w-full text-brand-2 bg-white border shadow-md hover:bg-brand-1 text-left py-6 px-4 rounded-lg"
                onClick={() => {
                  onSelect(sede.value);
                  onClose();
                }}
